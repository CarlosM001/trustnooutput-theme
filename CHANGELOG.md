...
- Extracted header interactions to `assets/header.js` (modular & cacheable).
- Accessible mobile menu using dialog semantics, focus trap, and ESC/outside-click close.
- Header transparency behavior maintained via `IntersectionObserver`.
...
- ARIA state synchronization on toggles and overlays (`aria-expanded`, `aria-hidden`).

### Notes

- Run Lighthouse (Mobile + Desktop) before publishing.
- If LCP depends on the hero image, consider adding `fetchpriority="high"`.
- Plan to include axe-core accessibility smoke tests in the next iteration.
...
[Unreleased]: https://github.com/CarlosM001/trustnooutput-theme/compare/v0.2.1...HEAD
[0.2.1]: https://github.com/CarlosM001/trustnooutput-theme/releases/tag/v0.2.1
